...
string const DATABASE_FOLDERNAME = "data.db";
...
void prepare() {
  // Make folder for stored database
  string command;
  command += "mkdir -p ";
  command += DATABASE_FOLDERNAME;
  system(command.c_str());

  // Checking exists count file
  string filePath = DATABASE_FOLDERNAME + "/" + COUNT_FILENAME;
  struct stat buffer;
...
class SparseTable {
 ...
 public:
  ...
  int query(int lowerBound, int upperBound) {
    ...
    int k = log2(upperBound - lowerBound + 1);
    return op[f](table[k][lowerBound], table[k][upperBound - (1 << k) + 1]);
  }
  bool save(string fileName) {
    prepare();
    fstream countFile(DATABASE_FOLDERNAME + "/" + COUNT_FILENAME,
                      ios::in | ios::out);
    int count = 0;
    if (countFile.good()) {
      countFile >> count;
    ...
    // Check file exist
    // Then uncount the file
    struct stat buffer;
    string filePath = DATABASE_FOLDERNAME + "/" + fileName;
    if (stat(filePath.c_str(), &buffer) == 0) count--;
...
class Command {
 ...
 public:
  Command(string tableName) : tableName(tableName) {}
...
class MakeCommand : public Command {
 ...
 public:
  MakeCommand(char *tableName, char *tableType, char **seq, size_t seqSize)
    ...
    for (int i = 0; i < seqSize; i++) this->seq.push_back(atoi(seq[i]));
  }
  void execute() {
    int f = 0;
    if (tableType == "MIN")
